/* ======================================================================= */
/*  OBSCURITY v3.0.1‑r1 — Hardened Custom Sponge Cipher (CONSTANT‑TIME)   */
/* ======================================================================= */
/*
...
#define ARGON2_MEMORY     (1<<19)     /* 512 MiB */
#define ARGON2_PARALLEL   4

/* Light mode */
#define ARGON2_MEMORY_LIGHT (64 * 1024)
...
static int encrypt_file(const char *inf, const char *outf, const char *pwd) {
    ...
    if (argon2id_hash_raw(t, mem, ARGON2_PARALLEL, pwd, strlen(pwd), salt, SALT_BYTES,
                          totkey, TOTAL_KEY_BYTES)) goto err;
...
static int decrypt_file(const char *inf, const char *outf, const char *pwd) {
    ...
    /* Read ciphertext, poly tag, expected HMAC */
    uint64_t ct_len = fsize - (SALT_BYTES+NONCE_BYTES+POLY1305_TAG_BYTES+HMAC_TAG_BYTES);
    ct_buf = malloc(ct_len + POLY1305_TAG_BYTES + HMAC_TAG_BYTES);
    if (!ct_buf) goto err;
    if (fread(ct_buf,1,ct_len+POLY1305_TAG_BYTES+HMAC_TAG_BYTES,in) != ct_len+POLY1305_TAG_BYTES+HMAC_TAG_BYTES) goto err;
...
static void print_version(void) {
    printf("Obscurity v3.0.1-r1 (hardened custom sponge, constant‑time ARX)\n");
    printf("Build: %s %s\n", __DATE__, __TIME__);
...
int main(int argc, char **argv) {
    ...
    /* ----- Elegant main menu ----- */
    const char *HDR = COLOR_CYAN "               OBSCURITY v3.0.1-r1 Hardened\n" COLOR_RESET
                      "------------------------------------------------------------\n";
